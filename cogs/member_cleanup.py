...
class MemberCleanup(commands.Cog):
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        try:
            ...
            await update_pinned_birthday_message(member.guild)
            logger.info(f"👋 Removed {member.display_name}'s birthday because they left the server and refreshed pinned message in {member.guild.name}")
